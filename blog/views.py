from rest_framework import status, permissions, serializers as rf_serializers
...
from .serializers import (
    ...
    AITitleGenerationSerializer,
    AIContentGenerationSerializer,
    AIContentRewriteSerializer,
)
...
class _LikeResponseSerializer(rf_serializers.Serializer):
    is_liked = rf_serializers.BooleanField()
    likes_count = rf_serializers.IntegerField()

class _SaveResponseSerializer(rf_serializers.Serializer):
    is_saved = rf_serializers.BooleanField()

class _ViewedResponseSerializer(rf_serializers.Serializer):
    viewed = rf_serializers.BooleanField()

class _SearchResponseSerializer(rf_serializers.Serializer):
    count = rf_serializers.IntegerField()
    results = BlogListSerializer(many=True)

class _AIBlogResponseSerializer(rf_serializers.Serializer):
    title = rf_serializers.CharField()
    content = rf_serializers.CharField()
    content_source = rf_serializers.CharField()
    ai_prompt = rf_serializers.CharField()

class _AITitleResponseSerializer(rf_serializers.Serializer):
    title = rf_serializers.CharField()
    tone = rf_serializers.CharField()
    original_prompt = rf_serializers.CharField()

class _AIContentResponseSerializer(rf_serializers.Serializer):
    content = rf_serializers.CharField()
    title = rf_serializers.CharField()
    tone = rf_serializers.CharField()
    length = rf_serializers.CharField()
    original_prompt = rf_serializers.CharField()

class _AIRewriteResponseSerializer(rf_serializers.Serializer):
    rewritten_content = rf_serializers.CharField()
    original_content = rf_serializers.CharField()
    tone = rf_serializers.CharField()
    style = rf_serializers.CharField()
    instruction = rf_serializers.CharField(allow_blank=True)

class _BlogErrorSerializer(rf_serializers.Serializer):
...
@extend_schema_view(
    list=extend_schema(
        tags=["Blogs"],
        summary="List all public blogs (+ own drafts if authenticated)",
        responses={200: BlogListSerializer(many=True)},
        parameters=[
            OpenApiParameter("author", str, description="Filter by author UUID"),
            OpenApiParameter("status", str, enum=["public", "draft"], description="Filter by status"),
            OpenApiParameter("tags", str, description="Comma-separated tag filter"),
            OpenApiParameter("search", str, description="Full-text search across title, content, tags, author"),
        ],
    ),
    create=extend_schema(
        ...
        responses={201: BlogListSerializer},
    ),
    retrieve=extend_schema(
        tags=["Blogs"],
        summary="Get a single blog post by UUID",
        responses={200: BlogSerializer},
    ),
    update=extend_schema(
        tags=["Blogs"],
        summary="Replace a blog post (full update)",
    ...
    like=extend_schema(
        ...
        responses={200: _LikeResponseSerializer},
    ),
    save=extend_schema(
        tags=["Blogs"],
        summary="Bookmark or unbookmark a blog post",
        responses={200: _SaveResponseSerializer},
    ),
    track_view=extend_schema(
        tags=["Blogs"],
        summary="Record a view for analytics",
        responses={200: _ViewedResponseSerializer},
    ),
    search=extend_schema(
        tags=["Blogs"],
        summary="Ranked full-text search across public blog titles and content",
        responses={200: _SearchResponseSerializer},
        parameters=[
            OpenApiParameter("q", str, required=True, description="Search string"),
...
class BlogViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        queryset = (
            ...
            .prefetch_related("likes", "comments", "saved_by_users")
            .annotate(
                likes_count=Count(
                    "likes", filter=Q(likes__is_liked=True), distinct=True
                ),
        ...
        if not self.request.user.is_authenticated:
            ...
            queryset = queryset.filter(Q(status="public") | Q(author=self.request.user))
    ...
    def search(self, request):
        ...
        if not query:
            return Response(
                {"detail": "Missing q parameter"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        return Response({"count": ranked_qs.count(), "results": serializer.data})
    ...
    def like(self, request, pk=None):
        ...
        like, created = Like.objects.get_or_create(
            blog=blog, user=request.user, defaults={"is_liked": True}
        )
...
@extend_schema_view(
    list=extend_schema(
        tags=["Comments"],
        summary="List top-level comments for a blog post",
    ...
    update=extend_schema(
        tags=["Comments"],
        summary="Replace a comment (full update)",
        request=CommentCreateSerializer,
        responses={200: CommentSerializer},
    ),
    partial_update=extend_schema(
...
class CommentViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if blog_id:
            ...
            if comment_pk:
                return (
                    Comment.objects.filter(blog_id=blog_id, pk=comment_pk)
            ...
            if self.action == "list":
                return (
                    Comment.objects.filter(blog_id=blog_id, parent_comment__isnull=True)
                    .select_related("author")
                    .prefetch_related("replies__author")
                    .order_by("-created_at")
                )
            ...
            return (
                Comment.objects.filter(blog_id=blog_id)
                .select_related("author")
                .prefetch_related("replies__author")
            )
...
@extend_schema(
    tags=["AI"],
    request=AIBlogGenerationSerializer,
    responses={200: _AIBlogResponseSerializer, 400: _BlogErrorSerializer, 500: _BlogErrorSerializer},
    summary="Generate a full blog post using OpenAI GPT",
)
@api_view(["POST"])
...
@extend_schema(
    tags=["AI"],
    request=AITitleGenerationSerializer,
    responses={200: _AITitleResponseSerializer, 400: _BlogErrorSerializer, 500: _BlogErrorSerializer},
    summary="Generate a blog title using OpenAI GPT",
)
@api_view(["POST"])
@permission_classes([permissions.IsAuthenticated])
def generate_ai_title(request):
    ...
    try:
        ...
        return Response(
            {"title": generated_title, "tone": tone, "original_prompt": prompt}
        )
...
@extend_schema(
    tags=["AI"],
    request=AIContentGenerationSerializer,
    responses={200: _AIContentResponseSerializer, 400: _BlogErrorSerializer, 500: _BlogErrorSerializer},
    summary="Generate blog body content using OpenAI GPT",
)
@api_view(["POST"])
@permission_classes([permissions.IsAuthenticated])
def generate_ai_content(request):
    ...
    try:
        ...
        max_tokens_map = {"short": 1000, "medium": 2000, "long": 3000}
...
@extend_schema(
    tags=["AI"],
    request=AIContentRewriteSerializer,
    responses={200: _AIRewriteResponseSerializer, 400: _BlogErrorSerializer, 500: _BlogErrorSerializer},
    summary="Rewrite existing blog content using OpenAI GPT",
)
@api_view(["POST"])
@permission_classes([permissions.IsAuthenticated])
...
@extend_schema(
    tags=["Blogs"],
    responses={200: SavedBlogSerializer(many=True)},
    summary="List all blogs bookmarked by the authenticated user",
)
@api_view(["GET"])
@permission_classes([permissions.IsAuthenticated])
def user_saved_blogs(request):
    ...
    """
    saved_blogs = (
        SavedBlog.objects.filter(user=request.user)
        .select_related("blog__author")
        .order_by("-created_at")
    )

    serializer = SavedBlogSerializer(
        saved_blogs, many=True, context={"request": request}
    )
...
@extend_schema(
    ...
    summary="List all blogs authored by the authenticated user",
)
@api_view(["GET"])
@permission_classes([permissions.IsAuthenticated])
...
@extend_schema(
    ...
    summary="Get blog statistics for the authenticated user (counts, views)",
)
@api_view(["GET"])
@permission_classes([permissions.IsAuthenticated])
...
@extend_schema(
    tags=["Blogs"],
    responses={200: BlogSerializer, 404: None},
    summary="Get blog detail by SEO-friendly slug (public)",
)
@api_view(["GET"])
...
def blog_detail_by_slug(request, slug):
    ...
    try:
        blog = (
            Blog.objects.select_related("author")
            .prefetch_related("likes", "comments__author", "saved_by_users")
            .annotate(
                likes_count=Count("likes", filter=Q(likes__is_liked=True)),
                comments_count=Count("comments"),
            )
    ...
    if blog.status != "public" and (
        not request.user.is_authenticated or blog.author != request.user
    ):
        raise Http404("Blog not found")
    ...
    if request.user.is_authenticated:
        ...
        BlogView.objects.get_or_create(
            blog=blog, user=request.user, defaults={"ip_address": ip_address}
        )
